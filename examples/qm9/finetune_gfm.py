...
parser = argparse.ArgumentParser(description="Fine-tune a GFM on QM9")
parser.add_argument("--split", type=int, required=True, help="Percentage of training data (e.g., 1, 3, 5, 10, 20, 100)")
parser.add_argument("--checkpoint", type=str, required=True, help="Path to GFM checkpoint .pk file")
...
def load_gfm_model(config, checkpoint_path):
    model = hydragnn.models.create_model_config(config=config["NeuralNetwork"], verbosity=config["Verbosity"]["level"])
    state = torch.load(checkpoint_path, map_location="cpu")
    model.load_state_dict(state["model_state_dict"], strict=False)
    print(" Loaded GFM checkpoint:", checkpoint_path)
    return model
...
log_name = f"finetune_gfm_{args.split}"
...
output_path = f"finetuned_gfm_qm9_{args.split}.pt"
...
print(f" Final Accuracy on {args.split}% split: R² = {r2:.4f}, MAE = {mae:.4f}")
